...
using InstaConnect.Users.Business.Query.User;
...
namespace InstaConnect.Business.Services
{
    public class UserService : IUserService
    {
        ...
        public async Task<IResult<ICollection<GetUserQuery>>> GetAllAsync(
            string firstName,
            string lastName,
            int page,
            int amount)
        {
            ...
            var userResultDTOs = _mapper.Map<ICollection<GetUserQuery>>(users);
            var okResult = _resultFactory.GetOkResult(userResultDTOs);

            return okResult;
        }

        public async Task<IResult<GetUserQuery>> GetByIdAsync(string id)
        {
            var existingUser = await _userRepository.FindEntityAsync(u => u.Id == id);

            if (existingUser == null)
            {
                var notFoundResult = _resultFactory.GetNotFoundResult<GetUserQuery>();

                return notFoundResult;
            }

            var userResultDTO = _mapper.Map<GetUserQuery>(existingUser);
            var okResult = _resultFactory.GetOkResult(userResultDTO);

            return okResult;
        }

        public async Task<IResult<GetUserQuery>> GetByUsernameAsync(string username)
        {
