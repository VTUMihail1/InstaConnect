...
namespace InstaConnect.Business.Services
{
    public class TokenService : ITokenService
    {
        ...
        public IResult<TokenAddDTO> GeneratePasswordResetToken(AccountResultDTO accountResultDTO)
        {
            var tokenGenerateDTO = _mapper.Map<TokenGenerateDTO>(accountResultDTO);
            var tokenAddDTO = _tokenHandler.GenerateForgotPasswordToken(tokenGenerateDTO);

            var okResult = _resultFactory.GetOkResult(tokenAddDTO);

            return okResult;
        }

        public IResult<TokenAddDTO> GenerateEmailConfirmationToken(AccountResultDTO accountResultDTO)
        {
            var tokenGenerateDTO = _mapper.Map<TokenGenerateDTO>(accountResultDTO);
            var tokenAddDTO = _tokenHandler.GenerateEmailConfirmationToken(tokenGenerateDTO);
        ...
        public IResult<TokenAddDTO> GenerateAccessToken(AccountResultDTO accountResultDTO)
        {   
            var tokenGenerateDTO = _mapper.Map<TokenGenerateDTO>(accountResultDTO);
        ...
        public async Task<IResult<TokenResultDTO>> AddAsync(TokenAddDTO tokenAddDTO)
        {
        ...
        public async Task<IResult<TokenResultDTO>> GetByValueAsync(string value)
        {
            if (value == null)
            {
                var unauthorizedResult = _resultFactory.GetUnauthorizedResult<TokenResultDTO>("Login");
